...
# Main function
def main(model_name, platform, debug=False):
    print(f"Using Model: {model_name}")
    logging.info(f"Using Model: {model_name}")

    # Set the role of the system: VD-Sys1
    conversation=[{"role": "system", "content": "You are an AI binary vulnerability classifier that identifies whether the provided code is vulnerable or not vulnerable. You should respond with either only 'vulnerable' or 'not vulnerable'."}]
    ...
    for l in languages:
        ...
        for item in data:
            ...
            classification = classify_vulnerability(conversation, client, platform, model_name)
...
if __name__ == "__main__":
    ...
    # Create an instance of AzureOpenAI if the platform is Azure, else create an instance of OpenAI
    if args.platform == "azure":
        ...
        if not success:
            ...
            logging.info("Azure OpenAI environment variables loaded successfully!")
        ...
        # Load environment variables from .env file based on the model name
        success = dotenv.load_dotenv(dotenv_path=f"../../credentials/.chat-openai.env")                             
        if not success:
            print("Could not read environment variables!")
        else:
            logging.info("OpenAI environment variables loaded successfully!")
            # Create an instance of OpenAI
            client = OpenAI(
                organization=os.getenv("OPENAI_ORG"),
                project=os.getenv("OPENAI_PROJECTID"),
                api_key=os.getenv("OPENAI_API_KEY"),
            )
    else:
        print("Invalid platform. Please choose either 'azure' or 'openai'.")


    # Run main function
    main(args.model_name, args.platform, args.debug)
